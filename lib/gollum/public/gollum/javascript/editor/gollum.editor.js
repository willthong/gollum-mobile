...
(function($) {
  ...
  function isMobileEditor() {
    return window.innerWidth < ActiveOptions.MobileThreshold;
  }
  ...
  function autoSaveHandler() {
    ...
    autoSaveTimer = setTimeout(function() {
      var content = isMobileEditor()
        ? $('textarea[id="gollum-editor-body"]').val()
        : window.ace_editor ? window.ace_editor.getSession().getValue() : '';
   ...
   */
  function injectMobileCssRules() {
    // Inject CSS rules that match the configured MobileThreshold so users
    // only need to configure the threshold in one place (the JS option).
    var threshold = ActiveOptions.MobileThreshold - 1;
    var css = [
  ...
  $.GollumEditor = function( IncomingOptions ) {
    ...
    // Inject dynamic CSS rules so the mobile breakpoint stays in sync
    injectMobileCssRules();

    var isMobile = isMobileEditor();
    ...
    $('textarea[id="gollum-editor-body"]').each(function () {
      ...
      if (isMobile) {
        // On mobile: keep the textarea visible, skip ACE editor entirely
        textarea.css('display', '');

        // Set up autosave on the textarea directly
        textarea.on('change keyup paste', function() {
          autoSaveHandler();
        });

        // Restore autosaved text if present
        ...
        // Desktop: create ACE editor, hide the textarea
        var editDiv = $('<div>', {
          position: 'absolute',
    ...
    if (!isMobile) {
      ...
      // On mobile, format selector changes just update the hidden field
      $("#wiki_format").on('change', function(event, selected_value) {
        $('#format').attr('value', selected_value);
        FormatSelector.updateCommitMessage(selected_value);
      });
    }
    ...
    if ( EditorHas.baseEditorMarkup() ) {
      ...
      if ( EditorHas.functionBar() ) {
        ...
        if ( EditorHas.dir() && !isMobileEditor() ) {
      ...
      if (ActiveOptions['section'] && $.markupSupportsEditableSections ( ActiveOptions.MarkupType ) && !isMobileEditor()) {
      ...
      if ( EditorHas.dragDropUpload() && !isMobileEditor() ) {
   ...
   */
   var FunctionBar = {
      ...
      refresh: function() {
        if ( EditorHas.functionBar() ) {
          // On mobile, keep the function bar deactivated (ACE not available)
          if (isMobileEditor()) {
            debug('Mobile device: skipping function bar activation');
            if ( FunctionBar.isShown() ) {
              FunctionBar.deactivate();
            }
            if ( Help && Help.isShown() ) {
              Help.hide();
            }
            return;
          }
